...
public class TestExercise1 extends ExerciseTest {
  ...
  public void testDivision() {
    Maths maths = new Maths();
    assertEquals("12/4 should equal 3", 3, maths.divide(12, 4), 0);
    assertEquals("-25/5 should equal -5", -5, maths.divide(-25, 5), 0);
    assertEquals("18/-3 should equal -6", -6, maths.divide(18, -3), 0);
    assertEquals("12/5 should equal 2 (remainder should be ignored)", 2, maths.divide(12, 5), 0);
  }


  /**
   * Bonus: let's see what happens if we try to do something that shouldn't work,
   * and also let's experiment with modifying the method a little.
   **/
  @Test
